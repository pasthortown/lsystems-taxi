...
@Component({
   selector: 'app-motivoestado',
   templateUrl: './motivoestado.component.html',
   styleUrls: ['./motivoestado.component.scss']
})

export class MotivoEstadoComponent implements OnInit {

   busy: Promise<any>;
   entidades: MotivoEstado[];
   entidadSeleccionada: MotivoEstado;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: MotivoEstadoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: MotivoEstado): boolean {
      return true;
   }
   ...
   crearEntidad(): MotivoEstado {
      const nuevoMotivoEstado = new MotivoEstado();
      nuevoMotivoEstado.id = 0;
      return nuevoMotivoEstado;
   }

   add(entidadNueva: MotivoEstado): void {
   ...
   update(entidadParaActualizar: MotivoEstado): void {
   ...
   delete(entidadParaBorrar: MotivoEstado): void {
   ...
   onSelect(entidadActual: MotivoEstado): void {
