...
@Component({
   selector: 'app-estadoviaje',
   templateUrl: './estadoviaje.component.html',
   styleUrls: ['./estadoviaje.component.scss']
})

export class EstadoViajeComponent implements OnInit {

   busy: Promise<any>;
   entidades: EstadoViaje[];
   entidadSeleccionada: EstadoViaje;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: EstadoViajeService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: EstadoViaje): boolean {
      return true;
   }
   ...
   crearEntidad(): EstadoViaje {
      const nuevoEstadoViaje = new EstadoViaje();
      nuevoEstadoViaje.id = 0;
      return nuevoEstadoViaje;
   }

   add(entidadNueva: EstadoViaje): void {
   ...
   update(entidadParaActualizar: EstadoViaje): void {
   ...
   delete(entidadParaBorrar: EstadoViaje): void {
   ...
   onSelect(entidadActual: EstadoViaje): void {
