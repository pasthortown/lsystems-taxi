...
export class PosicionComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: PosicionService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Posicion): boolean {
      return true;
   }
   ...
   crearEntidad(): Posicion {
      const nuevoPosicion = new Posicion();
      nuevoPosicion.id = 0;
      return nuevoPosicion;
   }

   add(entidadNueva: Posicion): void {
   ...
   update(entidadParaActualizar: Posicion): void {
   ...
   delete(entidadParaBorrar: Posicion): void {
   ...
   onSelect(entidadActual: Posicion): void {
