...
@Component({
   selector: 'app-fotografiapersona',
   templateUrl: './fotografiapersona.component.html',
   styleUrls: ['./fotografiapersona.component.scss']
})

export class FotografiaPersonaComponent implements OnInit {

   busy: Promise<any>;
   entidades: FotografiaPersona[];
   entidadSeleccionada: FotografiaPersona;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: FotografiaPersonaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: FotografiaPersona): boolean {
      return true;
   }
   ...
   crearEntidad(): FotografiaPersona {
      const nuevoFotografiaPersona = new FotografiaPersona();
      nuevoFotografiaPersona.id = 0;
      return nuevoFotografiaPersona;
   }

   add(entidadNueva: FotografiaPersona): void {
   ...
   update(entidadParaActualizar: FotografiaPersona): void {
   ...
   delete(entidadParaBorrar: FotografiaPersona): void {
   ...
   refresh(): void {
      ...
      this.entidades = FotografiaPersona[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: FotografiaPersona): void {
