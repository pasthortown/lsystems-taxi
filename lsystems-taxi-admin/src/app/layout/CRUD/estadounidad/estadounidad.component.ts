...
@Component({
   selector: 'app-estadounidad',
   templateUrl: './estadounidad.component.html',
   styleUrls: ['./estadounidad.component.scss']
})

export class EstadoUnidadComponent implements OnInit {

   busy: Promise<any>;
   entidades: EstadoUnidad[];
   entidadSeleccionada: EstadoUnidad;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: EstadoUnidadService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: EstadoUnidad): boolean {
      return true;
   }
   ...
   crearEntidad(): EstadoUnidad {
      const nuevoEstadoUnidad = new EstadoUnidad();
      nuevoEstadoUnidad.id = 0;
      return nuevoEstadoUnidad;
   }

   add(entidadNueva: EstadoUnidad): void {
   ...
   update(entidadParaActualizar: EstadoUnidad): void {
   ...
   delete(entidadParaBorrar: EstadoUnidad): void {
   ...
   onSelect(entidadActual: EstadoUnidad): void {
