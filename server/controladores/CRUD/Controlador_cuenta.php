...
class Controlador_cuenta extends Controlador_Base
{
   function crear($args)
   {
      $cuenta = new Cuenta($args["id"],$args["idPersona"],$args["idRol"],$args["clave"]);
      $sql = "INSERT INTO Cuenta (idPersona,idRol,clave) VALUES (?,?,?);";
      $parametros = array($cuenta->idPersona,$cuenta->idRol,$cuenta->clave);
   ...
   {
      $cuenta = new Cuenta($args["id"],$args["idPersona"],$args["idRol"],$args["clave"]);
      $parametros = array($cuenta->idPersona,$cuenta->idRol,$cuenta->clave,$cuenta->id);
      $sql = "UPDATE Cuenta SET idPersona = ?,idRol = ?,clave = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
