...
class Controlador_login extends Controlador_Base
{
   ...
   { 
       $email = $args["email"];
       $clave = $args["clave"];
       $sql = "SELECT Persona.*, Cuenta.idRol as 'idRol', Cuenta.idEstadoCuenta  FROM Persona INNER JOIN Cuenta ON Cuenta.idPersona = Persona.id WHERE Persona.correoElectronico = ? AND Cuenta.clave = aes_encrypt(?,'".CIFRADO."');";
       $parametros = array($email, $clave);
       $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
       if(is_null($respuesta[0])||$respuesta[0]==0){
          return new LoginResult(0,new Persona);
       }else{
           if($respuesta[0]["idEstadoCuenta"] != 2){
              return new LoginResult(0,new Persona);
           }
